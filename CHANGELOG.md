...
- `antibot.logging.store_database_events` can now be set via the `ANTIBOT_STORE_DATABASE_EVENTS` environment variable, matching how every other toggleable config key in `config/antibot.php` is already overridable from `.env`. Still defaults to `false` when unset.
...
- `RedisRateLimitStore::hit()` passed the sliding-window cutoff to `ZREMRANGEBYSCORE` as a raw `float` (`microtime(true) - $windowSeconds`). The phpredis extension types that command's range arguments as `string` (to support the `-inf`/`+inf`/`(exclusive` range syntax); under this package's `declare(strict_types=1)`, some phpredis builds throw a `TypeError` on the un-cast float instead of coercing it, taking down `RateAnalyzer` and (via the configured `failure_strategy`) silently fail-opening the whole risk-scoring pipeline. The cutoff is now explicitly cast to `string`. The in-memory test fake (`ArrayRedisConnection`) previously accepted `string|float` here, which is why the existing test suite never caught this — its `zremrangebyscore()` signature is now typed `string` to match real phpredis, and a dedicated `RedisRateLimitStoreTest` guards against a regression.
